...
public class EditorWidgeText : EditorWindow
{
    [MenuItem("Tools/Graphy/Test")]
    public static void CreateWindow()
    {
        GetWindow<EditorWidgeText>("WidgetTest");
    }

    private  GUIContent m_content = new GUIContent("Drop");
    private void OnGUI() 
    {
        if(EditorGUILayout.DropdownButton(m_content, FocusType.Passive)  ) 
        {
            Debug.Log("Dropped.");
        }
    }
...
public class MyWindow : EditorWindow
{
    ...
    private EFadeGroup fg ;
    void OnEnable()
    {
        m_ShowExtraFields = new AnimBool(true);
        m_ShowExtraFields.valueChanged.AddListener(Repaint);
        fg.AddWidget(new EButton("1"));
        fg.AddWidget(new EButton("2"));
        fg.AddWidget(new EButton("3"));
        fg.AddWidget(new EButton("4"));
        fg.AddWidget(new EButton("5"));
        fg.AddWidget(new EButton("6"));
        fg.showExtraFields.valueChanged.AddListener(Repaint);
        fg  = new EFadeGroup("FadeGroup",Repaint);
